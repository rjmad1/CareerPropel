    ...
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-4 gap-8 mb-12">
        ...
        <div className="bg-white p-6 rounded border border-gray-200">
          ...
          <p className="text-sm font-medium text-gray-900 mt-2">{toolCalls.length}</p>
        </div>
        <div className="bg-white p-6 rounded border border-gray-200">
          <p className="text-xs text-gray-600 font-semibold">Total Duration</p>
          <p className="text-sm font-medium text-gray-900 mt-2">{totalDuration}ms</p>
        </div>
        <div className="bg-white p-6 rounded border border-gray-200">
          <p className="text-xs text-gray-600 font-semibold">Tokens</p>
          <p className="text-sm font-medium text-gray-900 mt-2">{execution.tokenCount || 0}</p>
        </div>
      </div>
      ...
      {sortedToolCalls.length > 0 ? (
        <div className="space-y-4">
          <h3 className="font-semibold text-sm text-gray-900 mb-6">Execution Timeline</h3>
          ...
          {sortedToolCalls.map((toolCall, index) => (
            <div key={toolCall.id} className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              <div
                ...
                className={cn(
                  'p-6 cursor-pointer hover:bg-gray-50 transition-colors flex items-start gap-6',
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <span className={cn('text-xs px-4 py-0.5 rounded font-medium border', getToolStatusColor(toolCall.status))}>
                      {toolCall.status}
                    </span>
                ...
                <span className="text-gray-400 text-xs flex-shrink-0 mt-2">
              ...
              {expandedToolId === toolCall.id && (
                <div className="border-t border-gray-200 p-6 bg-gray-50 space-y-6">
                  {/* Timing */}
                  <div>
                    <p className="text-xs text-gray-600 font-semibold mb-2">Timing</p>
                  ...
                  {toolCall.input && (
                    <div>
                      <p className="text-xs text-gray-600 font-semibold mb-2">Input</p>
                      <pre className="text-xs bg-white p-4 rounded border border-gray-300 overflow-x-auto max-h-40 overflow-y-auto">
                  ...
                  {toolCall.output && (
                    <div>
                      <p className="text-xs text-gray-600 font-semibold mb-2">Output</p>
                      <pre className="text-xs bg-white p-4 rounded border border-gray-300 overflow-x-auto max-h-40 overflow-y-auto">
                  ...
                  {toolCall.error && (
                    <div>
                      ...
                      <p className="text-xs bg-red-50 p-4 rounded border border-red-300 text-red-700">
                        {toolCall.error}
                      </p>
                    </div>
                  )}

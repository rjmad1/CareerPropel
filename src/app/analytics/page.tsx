...
export default function AnalyticsPage() {
  ...
  // Match score distribution buckets (for chart)
  ...
  return (
    <NavLayout title="Analytics" subtitle="Pipeline performance, conversion rates, and salary insights">
      <div className="p-6 max-w-6xl mx-auto flex flex-col gap-6">
        ...
        {/* Tab Selection Row */}
        <div className="flex border-b border-slate-100 dark:border-slate-800 pb-px overflow-x-auto gap-2 scrollbar-none">
          {[
            ...
            {activeTab === 'pipeline' && (
              jobs.length === 0 ? (
                ...
                <div className="flex flex-col gap-6">
                  ...
                  {/* Pipeline Stage Chart (visual bar chart) */}
                  <Card data-cy="chart-pipeline-stages">
                    <CardBody className="p-6">
                      ...
                      <div className="flex flex-col gap-3" data-cy="stage-table">
                        <table className="min-w-full text-sm hidden">
                          <tbody>
                            {stageBreakdownEntries.map(([stage, count]) => (
                              <tr key={stage} data-cy={`stage-${stage}`}>
                        ...
                        {stageBreakdownEntries.map(([stage, count]) => (
                          <div key={stage} className="flex items-center gap-4 text-sm" data-cy={`stage-${stage}`}>
                            <span className="w-40 text-slate-650 dark:text-slate-405 font-medium truncate">
                        ...
                        {stageBreakdownEntries.length === 0 && (
                          <p className="text-sm text-slate-400 dark:text-slate-500 py-4 text-center">No stage data yet.</p>
                        )}
                      </div>
                    </CardBody>
                  </Card>
                          ...
                          <div key={label} className="flex flex-col items-center gap-1.5 flex-1">
                            ...
                            <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-t-lg overflow-hidden" style={{ height: '60px' }}>
                              <div
                                ...
                                style={{
                                  height: `${matchBuckets[i] > 0 ? Math.max(8, (matchBuckets[i] / maxMatchBucket) * 60) : 0}px`,
                                  marginTop: `${60 - (matchBuckets[i] > 0 ? Math.max(8, (matchBuckets[i] / maxMatchBucket) * 60) : 0)}px`,
                                }}
                              />
                            </div>
                        ...
                        {metrics.salaryMetrics.average > 0 ? (
                          <div className="flex flex-col gap-3.5">
                            {[
                              ...
                              <div
                                key={row.label}
                                className="flex justify-between items-center text-sm border-b border-slate-50 dark:border-slate-800 pb-2 last:border-b-0 last:pb-0"
                                data-cy={row.cy}
                              >
                                <span className={!row.bold ? 'text-slate-650 dark:text-slate-400' : 'text-slate-650 dark:text-slate-400'}>
                                  {row.label}
                                </span>
                                <span className={row.bold ? row.color : 'text-slate-905 dark:text-white font-semibold'}>
                        ...
                        <div className="flex flex-col gap-4">
                          {Object.entries(metrics.outcomesSummary).filter(([, v]) => v > 0).map(([key, count]) => (
                            <div key={key} className="flex items-center gap-4 text-sm">
                              <span className="w-24 text-slate-650 dark:text-slate-405 font-medium capitalize truncate">
                                {key}
                              </span>
                              ...
                              <span className="w-8 text-right font-bold text-slate-800 dark:text-slate-205">
                                {count}
                              </span>
                            </div>
                          ))}
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
                              {metrics.topCompanies.slice(0, 10).map((co) => (
                                <tr key={co.name} className="hover:bg-slate-50/50 dark:hover:bg-slate-850 transition-colors">
                                  <td
                                    className="px-6 py-3.5 whitespace-nowrap font-medium text-slate-800 dark:text-slate-202"
                                    data-cy={`company-${co.name}`}
                                  >
                                    {co.name}
                                  </td>
                                  ...
                                  <td
                                    className="px-6 py-3.5 whitespace-nowrap text-right font-semibold"
                                    data-cy={`company-${co.name}-success-rate`}
                                  >
                                    {co.successRate > 0 ? (

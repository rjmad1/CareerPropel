...
export function subscribeToAgentExecution(
  ...
  try {
    const sse = acquireSseSubscription(`/api/agent/execution/${executionId}/subscribe`);

    const unsubUpdate = sse.subscribe('execution:update', (msg) => {
      onUpdate({ type: 'execution:update', data: msg.data });
    });

    const unsubToolCall = sse.subscribe('toolcall:complete', (msg) => {
      onUpdate({ type: 'toolcall:complete', data: msg.data });
    });

    const unsubLog = sse.subscribe('log:new', (msg) => {
      onUpdate({ type: 'log:new', data: msg.data });
    });

    const unsubConn = sse.onConnectionChange((connected) => {

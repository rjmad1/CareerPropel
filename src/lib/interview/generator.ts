...
function generateCompanyResearch(
  ...
  return {
    ...
    size: company.employees?.total > 10000 ? 'enterprise' : company.employees?.total > 1000 ? 'mid-market' : company.employees?.total > 100 ? 'scale-up' : 'startup',
...
async function generateTechnicalPrep(job: Job, resume: string): Promise<TechnicalPrep> {
  const technologies = extractTechnologies(job.title, resume);
...
function generateCompensationGuide(
  ...
  const salaryData = job.salary as { min?: number; max?: number } | undefined;
  const marketMin = salaryData?.min || 150000;
  const marketMax = salaryData?.max || 250000;
